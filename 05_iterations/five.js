...
*/
/*
1.The forEach() method is an iterative method. It calls 
a provided callbackFn function once for each element in 
an array in ascending-index order.
2.forEach() always returns undefined and is not chainab
le. 
3.Therefore we use map() and other functions
There is no way to stop or break a forEach() loop other 
than by throwing an exception. If you need such behaviour, 
the forEach() method is the wrong tool.
4.forEach() do not allow chaining
*/